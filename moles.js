...
bn1.addEventListener('click', startGame); //When the button "Start" is clicked, the function startGame() is called and the game begins.
...
function startGame() {
    ...
    timerID = null;
    timerID = setInterval(()=>appear(8,1000,1), 2000);  //8 moles on level 1 will appear with the interval of 2 seconds. 
}
...
function level2() {
    level.innerHTML = 2;
    if (timerID == null) {
        timerID = setInterval(()=>appear(10,800,2), 1400); //10 moles on level 2 will appear with the interval of 1.4 sec 
    }
}
...
function level3() {
    level.innerHTML = 3;
    if (timerID == null) {
        timerID = setInterval(()=>appear(12,600,3), 1000); //12 moles on level 3 will appear with the interval of 1 sec 
    }
}
